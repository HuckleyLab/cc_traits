...
class Permutation(object):
    ...
    model -- the model estimator to be used in permutation test. Must have 
     scikit-learn interface.
    features -- numpy array (or similar) of samples and features. 
    target   -- numpy array (or similar) of target response variable
    evaluator -- method with signature (model, features, target) --> score which
     evaluates the model
    given a set of features and targets.
    test_size -- percentage of dataset to reserve for post-train testing in
     evaluator function.
    ...
    def _permute_target(self):
        ...
        random values from within the domain of the target. Returns permuted
         target array.
        """
